...
for i in range(len(target)):
	...
	for ct in cts:
		...
		for ct2 in cts:
			a = chr(ord(ct[i])^ord(ct2[i]))
			for j in available:
				for k in available:
					if ord(j)^ord(k) == ord(a):
						if j in likely_letters.keys():
							likely_letters[j] += 1
						else:
							likely_letters[j]=1
						if k in likely_letters.keys():
							likely_letters[k] += 1
						else:
							likely_letters[k] = 1
		...
		best_letter = ""
		ll = 0
		for a in likely_letters.keys():
			if likely_letters[a] > ll:
				for x in cts:
					if not chr(ord(a)^ord(x[i])^ord(ct[i])) in available:
						continue
				best_letter = a
				ll = likely_letters[a]

		if ll>likeliness:
			most_likely_key = ord(best_letter)^ord(ct[i])
			likeliness = ll
	key = key[:i]+chr(most_likely_key)+key[i+1:]
	c = chr(most_likely_key^ord(target[i]))
	if c or c == ' ':
		msg = msg[:i] + c + msg[i+1:]
